...
const DWAForm = ({ onBack, readOnly = false, initialData = null }) => {
  const { state, dispatch } = useStock();
  
  const [formData, setFormData] = useState({
    dwaNumber: initialData?.dwaNumber || '',
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      ...
      <h2 className="text-2xl font-bold mb-6 text-gray-900">{readOnly ? 'View DWA Entry' : 'Create DWA Entry'}</h2>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        <Input 
          ...
          onChange={e => !readOnly && setFormData({...formData, dwaNumber: e.target.value})} 
          required 
          disabled={readOnly}
        />
        <Input 
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-gradient-to-r from-blue-50 to-indigo-50 border-b-2 border-blue-200">
              ...
              {!readOnly && <th className="px-4 py-3 text-center text-xs font-bold text-blue-900 uppercase">Action</th>}
            </tr>
          </thead>
          <tbody>
            {addedItems.length > 0 ? (
              ...
              <tr>
                <td colSpan={readOnly ? "2" : "3"} className="px-4 py-10 text-center text-gray-400 italic">No items added to the list yet.</td>
              </tr>
            )}
          </tbody>
        </table>
